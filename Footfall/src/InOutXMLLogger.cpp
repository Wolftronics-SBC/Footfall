...
//----------------Private Methods-------------------------
void InOutXMLLogger::finalizeCurrentLogFile() {
    time_t now;
    time(&now);
    
    logHeader.logEndTime = now;
    
    //Generate Header for final file
    size_t pos;
    string val;
    int markerLength;
    ...
    //Write content to final log file
    fFile<<xmlHeader;
...
void InOutXMLLogger::prepForNextLogFile() {
